...
export default function Navbar({ cartCount, onOpenCart }) {
  ...
  // ESTADO PARA LA SESIÓN DE USUARIO
  const [userSession, setUserSession] = useState(null);

  // EFECTO: Detectar cambios en la sesión cuando se abre/cierra el modal
  useEffect(() => {
    const checkSession = () => {
      try {
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <button
              ...
              className={`p-2 hover:bg-slate-50 rounded-full transition-colors ${
                userSession ? "text-blue-600 bg-blue-50" : "text-slate-600"
              }`}
              title={userSession ? "Mi Cuenta" : "Ingresar"}
            >
              <User size={20} />
            </button>
